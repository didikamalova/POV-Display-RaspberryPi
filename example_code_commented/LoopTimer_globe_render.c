...
// This code uses information from RTTmeasure
...
LoopTimer::LoopTimer(std::string tag)
    ...
    , m_curr_idx(0)
{
  for (int i = 0; i < history_size; i++){ // history size is defined as 100 in header file
    m_last_times[i] = m_last_time_print; // time now/defined above
  }
}
...
{
    const auto curr_time = std::chrono::steady_clock::now(); // time now
